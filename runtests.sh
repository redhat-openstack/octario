...
if [ -z ${COMPONENT_PATH+x} ]; then
  if [ -z ${COMPONENT_URL+x} ]; then
    ...
    COMPONENT_PATH=$DIR/temp/component
...
pushd $IR_DIR >>/dev/null
  ...
  echo -e "INFO: infrared:`infrared --version` ansible:`python -c "import ansible; print(ansible.__version__)"`"
  ...
  ( infrared openstack --topology-nodes=tester:1 \
      ...
      --key-file=${KEY} \
      > ${DIR}/provision.log 
      # TODO: find a way to disable stdout redirection if DEBUG variable is true
  ...
  if [ "${DISABLE_CLEANUP:-false}" != "true" ]; then
      infrared openstack ${IR_CLOUD:-} --image='fake_image' --prefix=$PREFIX --cleanup=yes
  fi
