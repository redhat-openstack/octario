...
package: prepare
	$(PYENV_HOME)/bin/python -m tox -epackage
...
flake8: venv
	@echo "INFO:	flake8"
	$(PYENV_HOME)/bin/python -m tox -eflake8

test: prepare
	@echo "INFO:	test"
	$(PYENV_HOME)/bin/python -m tox

test-cli:
	$(PYENV_HOME)/bin/python -c "import $(PACKAGE_NAME); print($(PACKAGE_NAME).__version__)"
